...
def calculate_scores(map, pos, score, direction, visited):
    ...
    calculate_scores(
        map, get_next_pos(pos, clockwise_direction), score + 1, clockwise_direction, visited
    )
...
def out_of_bounds(map, new_pos):
    return new_pos[0] >= len(map) or new_pos[1] >= len(map) or new_pos[0] < 0 or new_pos[1] < 0
...
def part_0(map, start_pos, end_pos):
    ...
    visited = [start_pos]
    calculate_scores(map, get_next_pos(start_pos, DIRECTIONS[0]), 1, DIRECTIONS[0], visited)
    calculate_scores(map, get_next_pos(start_pos, DIRECTIONS[1]), 1, DIRECTIONS[1], visited)
    calculate_scores(map, get_next_pos(start_pos, DIRECTIONS[-1]), 1, DIRECTIONS[-1], visited)
